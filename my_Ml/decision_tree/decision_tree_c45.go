...
func (D *D_tree_c45) find_best_demension(target []string, feature [][]string) (int, string) {
	entropy_loss_list := make([]float64, D.P_size)
	best_feature_list := make([]string, D.P_size)
	for i := 0; i < D.P_size; i++ {
		best_feature_list[i], entropy_loss_list[i] = entropy.Max_c45_feature_string(target, feature[i])
	}
	max_loss := float64(0)
	max_feature := ""
	max_id := 0
...
//按照一个决策标准将数据集一分为二
func (D *D_tree_c45) divide_data(target []string, feature [][]string, divider map[int]string) ([]string, [][]string, []string, [][]string) {
	di_id := 0
	di_feature := ""
...
//树的中序遍历
func (D *D_tree_c45) search(node *TreeNode) {
...
//用递归方法建立一个决策树
func (D *D_tree_c45) build_a_tree(node_now *TreeNode, target []string, feature [][]string, deep int) {
...
func (D *D_tree_c45) judge_sample_point(node_this *TreeNode, sample_x []string) (string, map[string]float64) {
	temp_id := 0
	temp_feature := ""
...
func (D *D_tree_c45) Fit() {
	D.init()
	D.build_a_tree(D.root_node, D.Y, D.feature_matix, 0)

}

func (D *D_tree_c45) Search() {
	D.search(D.root_node)
}

func (D *D_tree_c45) Make_a_judge(sample_x []string) (string, map[string]float64) {
